  ...
  def __init__(self, base_kwargs):
    ...
    if not isinstance(pos, int) or pos < 0 or pos > 3:
      raise Exception('Position function returns an illegal value (either ' +
                      'not an integer or an integer out of range: %s' %
                      str(pos))
...
class CorruptValueKeyboard(CorruptValue):
  ...
  def __init__(self, **kwargs):
    ...
    if abs((self.row_prob + self.col_prob) - 1.0) > 0.0000001:
      raise Exception('Sum of row and column probabilities does not sum '
                      'to 1.0')
...
class CorruptValueOCR(CorruptValue):
  ...
  def __init__(self, **kwargs):
    ...
    for rec_list in lookup_file_data:
      if (len(rec_list) != 2):
        raise Exception('Illegal format in OCR variations lookup file %s: %s' %
...
class CorruptCategoricalValue(CorruptValue):
  ...
  def __init__(self, **kwargs):
    ...
    for rec_list in lookup_file_data:
      ...
      if misspell_val == '':
        raise Exception('Empty misspelled attribute value in lookup file %s' %
                        (self.lookup_file_name))
      if org_val == misspell_val:
        raise Exception('Misspelled value is the same as original value in lookup file %s' %
                        (self.lookup_file_name))
...
class CorruptBirthYear(CorruptValue):
  ...
  def __init__(self, **kwargs):
    ...
    CorruptValue.__init__(self, base_kwargs=base_kwargs)
...
class CorruptDataSet:
  ...
  def __init__(self, **kwargs):
    ...
    for attr_name in self.attr_mod_prob_dict:
      ...
      if attr_name not in self.attribute_name_list:
        raise Exception('Attribute name "%s" in "attr_mod_prob_dict" not '
                        'listed in "attribute_name_list"' % attr_name)
    ...
    for (attr_name, attr_mod_data_list) in self.attr_mod_data_dict.items():
      ...
      if (abs(prob_sum - 1.0) > 0.0000001):
        raise Exception('Probability sum is not 1.0 for attribute "%s"' %
                        attr_name)
  ...
  def corrupt_records(self, rec_dict, verbose=True):
...
# =============================================================================
  ...
  def corrupt_names(self, rec_dict:dict, type: str = "switch", prob: float=0.5, first_col_index=None, second_col_index=None, third_col_index=None):
    ...
    params:
      rec_dict : dict obtained from the generator
      ...
      third_col_index : index of third attribute which should be corrupted

    returns : corrupted dict
    """

    if first_col_index == None or second_col_index == None:
      raise ValueError("Indexes for columns to corrupt not given.")
